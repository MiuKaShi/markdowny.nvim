...
---@nodiscard
local get_line = function(line_num) return vim.api.nvim_buf_get_lines(0, line_num - 1, line_num, false)[1] end

-- Delete a line from the buffer, 1-indexed.
---@param line_num integer The number of the line to be deleted.
---@nodiscard
local delete_line = function(line_num) vim.api.nvim_buf_set_lines(0, line_num - 1, line_num, true, {}) end
...
local set_mark = function(mark, position)
  if position then vim.api.nvim_buf_set_mark(0, mark, position[1], position[2] - 1, {}) end
end
...
local get_first_byte = function(pos)
  ...
  if not byte then return pos end
...
local inline_surround = function(before, after, opts)
  ...
  if vim.fn.visualmode() == "V" then e[2] = #get_line(e[1]) end
  ...
  if is_removing then
    ...
    if is_sameline then e[2] = e[2] + #before end
...
function M.bold() inline_surround("**", "**") end

function M.italic() inline_surround("_", "_") end
...
function M.setup(opts)
  ...
  vim.api.nvim_create_autocmd("FileType", {
    ...
    callback = function()
      if opts.remove_keymaps then return end
